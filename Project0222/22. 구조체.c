...
//구조체(struct)
//여러 자료형의 변수들을 하나로 묶어서 새로운 자료형으로 만드는 것을 의미(복합 자료형)
//사용자의 편의대로 묶어서 하나의 형태로 표현하는 방식이기에 사용자 정의 타입이라고도 부릅니다.
//데이터의 구조화 작업을 진행할때 사용하는 데이터 입니다.
...
//구조체 선언은 변수 선언은 아니기때문에, 사용하기위해서 따로 변수 선언와 초기화를 진행해야합니다.
int main()
{
	//구조체 변수 및 초기화 방법
	//struct 구조체명 변수명 = {구조체에 정의된 변수 순서대로 값을 작성합니다.};
	struct student s1 = { 1,"kate",'A' };

	struct student s2; //구조체 변수 선언
	
	//구조체 변수명.변수명 = 값;을 통해 수정 사능
	s2.number = 2;
	strcpy(s2.name, "Nero");
	...
	printf("이름 : ");
	scanf("%s", s3.name);
	getchar(); //문자열 다음에 문자를 받고있으니 geetchar를 통해 버퍼 비우기
	printf("그룹 : ");
	scanf("%c", &s3.group);
