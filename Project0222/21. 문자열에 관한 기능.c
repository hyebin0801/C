...
int main()
{
	...
	//% : 서식 지정자
	//l : long(정수)
	//u : unsigned(부호가 다로 없습니다. 숫자가 음수가 아님을 표현)
	...
	//함수 3. strncpy()
	//strncpy(바꿀 문장, 바꾸려는 문장, 갯수);를 통해 문자열을 바꾸려는 문장 중 일부의 값만 가지고 오는게 가능합니다. 
	char sample3[] = "HelloEveryone";
	char sample4[20];
	strncpy(sample4, sample3, 5); //비어있는 값에다 복사해줄 경우 이상한 값이 들어가게됨.
	sample4[5] = '\0'; //마지막 부분을 \0으로 처리합니다.
	printf("%s\n", sample4);

	//함수 4. strcat()
	//strcat(문자열1, 문자열2)을 통해 문자열 1에 문자열 2를 합칠 수 있음.
	char sample5[20] = "C언어";
	char sample6[] = "프로그래밍";
	strcat(sample5, sample6); //합칠 경우 문자열1쪽이 길이가 더 커야 오류가 없음.
	//만약 앞 단어가 더 작은 경우라면 배열에 길이라도 작성해서 더 큰 저장소임을 알려야함.
	printf("%s\n", sample5);

	//함수 5. strcmp()
	//strcmp(문자열1, 문자열2)
	//두 문자열을 비교합니다. 비교 결과에따라 다음과 같은 값을 return합니다.
	//0 : 두문자열은 서로 같습니다.
	//-1 : 서로 다릅니다.
	...
	//이 함수의 용도?strcmp로 비교한 결과 값이 0이면 같다는 점을 이용해서 문자열에대한 확인 용도
